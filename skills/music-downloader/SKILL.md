...
description: "基于 musicdl 匿名搜索、自动选择、下载和检查音乐文件，适用于用户希望用一句话下载歌曲、专辑或歌单的个人非商业场景。不用于绕过 DRM、付费、订阅、地区或账号访问限制。"
---
...
1. 用户给出明确的“歌手 + 歌名”时，先运行 `search`，再自动选择歌名和歌手完全匹配的结果并运行 `download`，不要为了常规的同源或格式差异中断一句话流程
2. 同一录音存在多个格式时，优先用户指定的格式；用户未指定时可优先选择 FLAC 容器，其次参考标称码率和文件大小，但不得据此宣称是真无损
3. 只有结果包含不同歌手、现场版、伴奏、翻唱或重混等实质性歧义时，才展示带编号候选并询问用户
4. 收到歌单链接时，先运行 `playlist` 生成清单并报告曲目数量；用户已经明确要求整张歌单时直接下载，否则确认下载范围
5. 下载后对所选文件运行 `inspect`，最后报告工具返回的真实文件名、绝对路径以及下载失败的项目

展示候选时只陈述平台、格式、文件大小、时长和码率等可验证信息。不要把平台返回的结果称为“正版”，也不要仅凭格式或文件大小使用“无损”“音质最好”等结论；需要帮助用户比较时，说明具体差异并让用户选择。
...
本技能不读取浏览器 Cookie、不接受账号凭证，也不提供登录模式。网易云和 QQ 音乐适配器在匿名模式下可能调用 `musicdl` 内置的第三方解析服务；这是本技能的默认行为，不要为此单独中断下载流程要求用户确认。
