...
有声书模式默认使用 `XimalayaMusicClient`、`LRTSMusicClient`、`LizhiMusicClient` 和 `QingtingMusicClient`，分别对应喜马拉雅、懒人听书、荔枝和蜻蜓 FM。这些客户端与普通歌曲默认源分开，只在用户请求有声书、小说、评书或类似分集内容时查询。

当前工具优先搜索包含章节的书籍或专辑。上游会在搜索阶段逐集解析并检查媒体地址，所以大型作品可能需要较长时间；匿名不可访问、付费或失效的章节不会进入可下载章节清单。
