...
这是一个基于 [musicdl](https://github.com/CharlesPikachu/musicdl) 的 Codex 技能，可用于个人非商业场景下的音乐、有声书和评书匿名搜索、候选选择、下载和音频信息检查。
...
用户回复 `1` 或 `2` 后，技能使用同一份短名单下载对应文件。

下载有声书时，可以直接说：

> 帮我下载有声小说《三体》

技能会改用喜马拉雅、懒人听书、荔枝和蜻蜓 FM 等有声内容源，先让用户选择匹配的书籍或专辑，再展开章节。用户可以选择单集、`3-10` 这样的章节范围或全部章节；下载整部作品前会先报告可下载章节数量和合计大小。

技能会先把分散在完整搜索结果中的匹配项生成连续编号短名单。例如原始结果编号为 `1、7、12、17` 时，展示给用户的短名单固定为 `1、2、3、4`；用户回复 `3` 就会下载原始第 `12` 项，不会发生序号错位。
...
- 有声书、小说和评书请求会自动改用专门的有声内容源，并按“书籍候选 → 章节候选”两级选择
